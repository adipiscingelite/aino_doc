...
func AddAsset(c echo.Context) error {
	...
	fmt.Println("c", c)
	fmt.Println("req", addAssetRequest)
	...
	fmt.Println("req after bind:", addAssetRequest) // Log setelah bind
	...
	userID := c.Get("user_id").(int) // Mengambil userUUID dari konteks
	userName := c.Get("user_name").(string)
	// addAssetRequest.FormData.UserID = userID
	addAssetRequest.AssetData.CreatedBy = userName
	// addAssetRequest.FormData.isProject = false
	// addAssetRequest.FormData.projectCode =
	// Token yang sudah dideskripsi
	fmt.Println("Token yang sudah dideskripsi:", decrypted)
	fmt.Println("User ID:", userID)
	fmt.Println("User Name:", userName)
	...
	// Validasi spasi untuk Code, Name, dan NumberFormat
	// whitespace := regexp.MustCompile(`^\s`)
	// if whitespace.MatchString(addAssetRequest.FormData.FormTicket) || whitespace.MatchString(addAssetRequest.FormData.FormNumber) {
	// 	return c.JSON(http.StatusUnprocessableEntity, &models.Response{
	// 		Code:    422,
	// 		Message: "Ticket atau Nomor tidak boleh dimulai dengan spasi!",
	// 		Status:  false,
	...
	errVal := c.Validate(&addAssetRequest.AssetData)
	//	addAssetRequest.FormData.UserID = userID
	if errVal == nil {
		// Gunakan addAssetRequest.IsPublished untuk menentukan apakah menyimpan sebagai draft atau mempublish
		addroleErr := service.AddAsset(addAssetRequest.AssetData, addAssetRequest.DataPIC, userID, userName, divisionCode, recursionCount)
		...
		return c.JSON(http.StatusCreated, &models.Response{
			Code:    201,
			Message: "Berhasil menambahkan formulir Asset!",
			Status:  true,
		})
...
func UpdateAsset(c echo.Context) error {
	...
	var updateAssetRequest struct {
		AssetData models.Asset `json:"assetData"`
		DataPIC   []models.Pic `json:"data_pic"`
	}

	fmt.Println("data update jawir", updateAssetRequest)
	fmt.Println("data update yg aset", updateAssetRequest.AssetData)

	if err := c.Bind(&updateAssetRequest); err != nil {
		log.Print("error saat binding:", err)
		return c.JSON(http.StatusBadRequest, &models.Response{
			Code:    400,
			Message: "Data tidak valid!",
	...
	updateAssetRequest.AssetData.UpdatedBy = updatedBy
	...
	if err := c.Validate(&updateAssetRequest.AssetData); err != nil {
		return c.JSON(http.StatusInternalServerError, &models.Response{
			Code:    422,
			Message: "Data tidak boleh kosong!",
			Status:  false,
	...
	previousContent, errGet := service.GetSpecAsset(id)
	if errGet != nil {
		log.Print(errGet)
		return c.JSON(http.StatusNotFound, &models.Response{
			Code:    404,
			Message: "Gagal mengupdate asset. Asset tidak ditemukan!",
			Status:  false,
		})
	}

	_, errService := service.UpdateAsset(updateAssetRequest.AssetData, updateAssetRequest.DataPIC, userName, userID)
	if errService != nil {
		log.Println("Kesalahan selama pembaruan:", errService)
		if errService.Error() == "You are not authorized to update this form" {
			return c.JSON(http.StatusUnauthorized, map[string]interface{}{
...
func DeleteAsset(c echo.Context) error {
	...
	id := c.Param("id")
	fmt.Println("aidi contr", id)
	perviousContent, errGet := service.GetSpecAllAsset(id)
	if errGet != nil {
		log.Print(errGet)
		return c.JSON(http.StatusNotFound, &models.Response{
			Code:    404,

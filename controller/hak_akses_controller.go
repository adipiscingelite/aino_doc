...
func UpdateHakAkses(c echo.Context) error {
	var updateFormRequest struct {
		FormData  models.HAReq             `json:"formData"`
		Signatory []models.Signatory    `json:"signatories"`
		InfoHA    []models.HakAksesRequest `json:"hakAksesInfoData"`
	}
	...
	id := c.Param("id")
	perviousContent, errGet := service.GetSpecHakAksesReq(id)
	if errGet != nil {
...
func UpdateHakAksesReview(c echo.Context) error {
	...
	// Periksa hasil dari service
	errService := service.UpdateHakAksesReview(id, userName, updateFormRequest.FormData, updateFormRequest.Signatory, updateFormRequest.InfoHA)
	if errService != nil {
		log.Println("Error during update:", errService)
		return c.JSON(http.StatusInternalServerError, &models.Response{
			Code:    500,
